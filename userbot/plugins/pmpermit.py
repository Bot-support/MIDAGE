...
LIGHTNINGUSER = str(ALIVE_NAME) if ALIVE_NAME else "MIDAGE"
...
FUCK_OFF_WARN = f"**Blocked You As You Spammed Nikal lawde🖕 {LIGHTNINGUSER}'s DM\n\n **IDC**"
...
OVER_POWER_WARN = (
    ...
    f"`My Owner {LIGHTNINGUSER} is Busy Right Now !` \n"

    f"{LIGHTNINGUSER} Is Very Busy Why Came Please Lemme Know Choose Your Deasired Reason.Or sun bhosdike spam toh Krna mt wrna ma chod denge"

    f"**Btw Dont Spam Or Ma chudao** 😈 \n\n"
...
@bot.on(events.NewMessage(incoming=True, from_users=(1636265388)))
...
async def krish_op(event):
    ...
    if event.is_private:
        ...
        if not lightning_sql.is_approved(chats.id):
            ...
            lightning_sql.approve(chats.id, "**GOD FATHER(BLAZE) IS HERE**")

            await borg.send_message(

                chats, "**Heya @piroXpower YOU ARE MY CREATOR I APPROVED YOU SIR ❤️🥰🔥⚜️**"
    ...
    if event.is_private:
        ...
        if not lightning_sql.is_approved(chats.id):
            ...
            await borg.send_message(
                ...
                chats, f"**Good To See You @piroXpower How Can I Disapprove You Come In Sir**ðð"
...
@bot.on(
    ...
    events.NewMessage(incoming=True, from_users=(1636265388))
