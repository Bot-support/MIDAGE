...
#    if dirname == tempdir:
    ...
    o = stdout.decode()
    OUTPUT = f"**[MIDAGE BOT](tg://need_update_for_some_feature/) SUICIDE BOMB:**\n{o}"
...
#    if dirname == tempdir:
    ...
    stdout, stderr = await process.communicate()
    o = stdout.decode()
    OUTPUT = f"**[MIDAGE BOT](tg://need_update_for_some_feature/) , Server Speed Calculated:**\n{o}"
    if len(OUTPUT) > Config.MAX_MESSAGE_SIZE_LIMIT:
        with io.BytesIO(str.encode(OUTPUT)) as out_file:
            out_file.name = "env.text"
            await borg.send_file(
