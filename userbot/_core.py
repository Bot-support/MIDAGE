...
DEFAULTUSER = str(ALIVE_NAME) if ALIVE_NAME else "MIDAGE BOT"
...
async def unload(event):
    ...
    try:
        ...
        qwe = await eor(event, f"MIDAGE BOT Has Successfully unloaded {shortname}")
    except Exception as e:
        await qwe.edit(
            "MIDAGE BOT has Successfully unloaded {shortname}\n{}".format(shortname, str(e))
        )
...
async def load(event):
    ...
    try:
        ...
        await qwe.edit(
            f"midage BOT could not load {shortname} because of the following error.\n{str(e)}"
        )
